...
class UsuarioDetalleController {
  async obtenerLugaresDelPropietario(req, res) {
    try {
      const { id } = req.params;
      ...
      const esPropietario = await usuarioDetalleService.verificarUsuarioEsPropietario(id);
      if (!esPropietario) {
...
module.exports = new UsuarioDetalleController();
